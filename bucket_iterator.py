...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        for data in batch_data:
            graph =data['sentic_graph']
            if len(graph) > temp:
                temp = len(graph)
        for data in batch_data:
            graph1 =data['dependency_graph']
            if len(graph1) > temp1:
                temp1 = len(graph1)
        ...
        for item in batch_data:
            ...
            batch_dependency_graph.append(numpy.pad(dependency_graph, ((0,max_len2-len(dependency_graph)),(0,max_len2-len(dependency_graph))), 'constant'))
            batch_sentic_graph.append(numpy.pad(sentic_graph, ((0,max_len1-len(sentic_graph)),(0,max_len1-len(sentic_graph))), 'constant'))
